...
import utils.JavascriptExecutorDriver;
...
public class Test_Case_10_Verify_Subscription_in_home_page {
	...
	public void setUp() {
		System.out.println("Test_Case_10_Verify_Subscription_in_home_page");
		b = new base();
	...
	@Test(enabled = true)
	private void s04_Scroll_down_to_footer() {
		jse.ScrollToElement(hwe.getFooterElement());
	}
	
	@Test(enabled = true)
	private void s05_Verify_text_SUBSCRIPTION() {
		a.AssertEqualsText(hwe.getSubscriptionLabelElement().getDomProperty("innerText"), "SUBSCRIPTION");
	}
	
	@Test(enabled = true)
	private void s06_Enter_email_address_in_input_and_click_arrow_button() {
		hwe.getEmailInputElement().sendKeys("chinchin@gmail.com");
		hwe.getSubmitButtonElement().click();
	}
	
	@Test(enabled = true)
	private void s07_Verify_success_message_You_have_been_successfully_subscribed_is_visible() {
